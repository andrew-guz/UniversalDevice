...
void MotionRelayWidget::OnSettingsButton() {
    ...
    if (result != 200)
        LOG_ERROR << fmt::format("Failed to update settings to {}.", nlohmann::json(newSettings).dump()) << std::endl;
}
...
void MotionRelayWidget::OnStateButton() {
    ...
    if (result != 200)
        LOG_ERROR << fmt::format("Failed to update commands to {}.", nlohmann::json(newCommands).dump()) << std::endl;
