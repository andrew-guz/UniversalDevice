...
Uuid::Uuid(const Uuid& other) { uuid_copy(_uuid, other._uuid); }

Uuid::Uuid(const std::string_view str) { uuid_parse(str.data(), _uuid); }
...
bool Uuid::operator==(const Uuid& other) const { return uuid_compare(_uuid, other._uuid) == 0; }
...
bool Uuid::isEmpty() const { return uuid_is_null(_uuid); }
