...
Platform::Platform(CrowApp& app, IQueryExecutor* queryExecutor) :
    ...
    _universalValuesController(queryExecutor),
    _cleanupController(queryExecutor, _thermometerValuesController, _relayValuesController, _motionRelayValuesController, _universalValuesController),
    _mainService(app),
    _settingsService(app, _settingsController),
    _commandsService(app, _commandsController),
    _deviceService(app, _devicesController),
    _deviceWebsocketsService(app),
    _eventsService(app, _eventsController),
    _scenariosService(app, _scenariosController, _eventsController),
    _backendLogsService(app) {}
...
Processors Platform::CreateProcessors(const Message& message) {
    ...
    switch (message._header._subject) {
        ...
        case Subject::TimerEvent:
            ...
            processors.push_back(std::shared_ptr<IProcessor>(new EventsProcessor(s_instance->_eventsController, s_instance->_commandsController)));
            // send time to devices
            processors.push_back(std::shared_ptr<IProcessor>(new TimeProcessor(s_instance->_cleanupController)));
            break;
        case Subject::GetDeviceInformation:
            ...
            processors.push_back(std::shared_ptr<IProcessor>(new ThermometerProcessor(s_instance->_thermometerValuesController)));
            processors.push_back(std::shared_ptr<IProcessor>(new RelayProcessor(s_instance->_relayValuesController)));
            processors.push_back(std::shared_ptr<IProcessor>(new MotionRelayProcessor(s_instance->_motionRelayValuesController)));
            processors.push_back(std::shared_ptr<IProcessor>(new UniversalDeviceProcessor(s_instance->_universalValuesController)));
            break;
        case Subject::ThermometerCurrentValue:
            // register thermometer if needed
            processors.push_back(std::shared_ptr<IProcessor>(new DeviceRegistrationProcessor(s_instance->_devicesController)));
        ...
        case Subject::WebSocketGetCommands:
            ...
            processors.push_back(
                std::shared_ptr<IProcessor>(new WebSocketProcessor(s_instance->_settingsController, s_instance->_commandsController)));
            break;
